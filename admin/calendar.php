...
$sql = "SELECT a.appt_id, a.appt_date, a.status, u_patient.name AS patient_name, u_doctor.name AS doctor_name 
        FROM appointments a
...
while ($row = $result->fetch_assoc()) {
    ...
    if ($row['status'] === 'Completed') $color = '#3b82f6'; // Blue
    if ($row['status'] === 'Cancelled') $color = '#ef4444'; // Red
    ...
    $events[] = [
        ...
        'color' => $color,
        'url' => 'appointments.php?search=' . urlencode($row['patient_name'])
    ];
}
...
<title>Calendar — MediCare HMS</title>
      ...
      <div class="card calendar-card">
        <div class="calendar-toolbar">
          <div class="calendar-summary">
            <strong><?= $eventCount ?> scheduled appointment<?= $eventCount === 1 ? '' : 's' ?></strong>
            <span>All departments and providers</span>
          </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
      initialView: window.innerWidth < 720 ? 'timeGridDay' : 'dayGridMonth',
      headerToolbar: {
